...
export function BrandWorldSection() {
  ...
  return (
    <section
      id="brand"
      className="scroll-mt-36 overflow-hidden bg-gradient-to-b from-pop-pink/10 via-wash-blush to-sky/15 py-14 sm:scroll-mt-40 sm:py-20"
    >
      <Container>
        <Reveal>
      ...
      <div className="relative mt-10">
        <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-24 bg-gradient-to-r from-wash-blush to-transparent" />
        <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-24 bg-gradient-to-l from-wash-blush to-transparent" />
        <motion.div
          className="flex gap-10 whitespace-nowrap px-6 text-xs font-semibold uppercase tracking-[0.35em] text-ink-muted sm:text-sm"
          animate={
            reduce
              ? undefined
              : { x: ["0%", "-50%"] }
          ...
          {[...marquee, ...marquee].map((word, i) => (
            <span key={`${word}-${i}`} className="flex items-center gap-10">
              ...
              <span
                className={
                  i % 3 === 0
                    ? "h-1.5 w-1.5 rounded-full bg-pop-pink"
                    : i % 3 === 1
                      ? "h-1.5 w-1.5 rounded-full bg-sky"
                      : "h-1.5 w-1.5 rounded-full bg-sun"
                }
                aria-hidden
              />
            </span>
